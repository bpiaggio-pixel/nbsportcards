...
function categoryLabel(category: string, locale: string) {
  switch (category) {
    case "pokemon":
      return "Pokemon";
    case "soccer":
      return locale === "es" ? "Soccer" : "Soccer";
    case "basketball":
      return "Basketball";
    case "nfl":
      return "NFL";
...
export async function generateMetadata({
  ...
  const path = `/${locale}/blog/${category}`;
  const title =
    locale === "es"
      ? `Blog ${categoryLabel(category, locale)}`
      : `${categoryLabel(category, locale)} Blog`;
  const description =
    locale === "es"
      ? `Noticias, guías y destacados de ${categoryLabel(category, locale)}.`
      : `News, guides, and highlights about ${categoryLabel(category, locale)}.`;
  ...
  return {
    ...
    alternates: {
      canonical: `${base}${path}`,
      languages: {
        en: `${base}/en/blog/${category}`,
        es: `${base}/es/blog/${category}`,
      },
    },
    openGraph: {
      type: "website",
