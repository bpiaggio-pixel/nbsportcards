...
function getRelativeForwardPath(...parts: string[]) {
...
function getByPath(obj: any, pathValue: string) {
  return pathValue.split(".").reduce((acc, key) => acc?.[key], obj);
}

function extractPriceFromAlgoliaHit(hit: any) {
  ...
  for (const candidate of candidatePaths) {
    const value = getByPath(hit, candidate);
  ...
  const currency =
    hit?.price?.currency ||
    hit?.listingPrice?.currency ||
    hit?.buyNowPrice?.currency ||
    hit?.currentPrice?.currency ||
    hit?.askingPrice?.currency ||
    hit?.fixedPrice?.currency ||
    hit?.salePrice?.currency ||
    "USD";
...
async function main() {
  ...
  for (const listing of listings) {
    ...
    const { price, currency } = extractPriceFromAlgoliaHit(listing);

    const sport = detectSport(title);
    const productUrl = `https://www.fanaticscollect.com/buy-now/${id}`;
    const baseName = `${slug || id}-${id}`;

    const cachedBackFile =
      [...existingBackFiles].find((name) => name.startsWith(`${baseName}-back.`)) || "";

    const hasCachedBack = Boolean(cachedBackFile);
    ...
    const backExt = backImage
      ...
      : hasCachedBack
      ? path.extname(cachedBackFile) || ".jpg"
      : ".jpg";

    const frontFile = frontImage ? `${baseName}-front${frontExt}` : "";
    const backFile = backImage
      ? `${baseName}-back${backExt}`
      : hasCachedBack
      ? cachedBackFile
      : "";

    const frontPath = frontFile
      ? getRelativeForwardPath("data", "fanatics", "front", frontFile)
      : "";

    const backPath = backFile
      ? getRelativeForwardPath("data", "fanatics", "back", backFile)
      : "";

    const hasBack = Boolean(backImage || hasCachedBack);

    console.log(`\n[${processed}/${listings.length}] ${title}`);
    console.log(`   🏷️  sport: ${sport}`);
    console.log(`   💵 price: ${price ?? "faltante"} ${currency}`);
    console.log(`   🔗 ${productUrl}`);
    console.log(`   🖼️  front: ${frontFile ? "ok" : "faltante"}`);
    console.log(
      `   🖼️  back:  ${hasBack ? "ok" : "faltante"}${hasCachedBack && !backImage ? " (cache)" : ""}`
    );
  ...
  const csvLines = [
    ...
    ...rows.map((row) =>
      [
        ...
        csvEscape(row.frontPath),
        csvEscape(row.backPath),
